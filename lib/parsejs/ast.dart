...
abstract class Scope extends Node {
  ...
  final variables = Set<(String,Node)>();
}
...
/// SpreadElement
class SpreadElement extends Property {
  Name argument;

  SpreadElement(this.argument) : super(Name(''), LiteralExpression(''));

  forEach(callback) {}

  String toString() => 'SpreadExpression';

  visitBy<T>(Visitor<T> v) => v.visitSpreadElement(this);
  visitBy1<T, A>(Visitor1<T, A> v, A arg) => v.visitSpreadElement(this, arg);
}
...
class ScopeArea {
  ...
  ScopeArea? parent;
  Scope scope;
  final children = <ScopeArea>[];

  ScopeArea(this.node, {required this.scope, this.parent});
}